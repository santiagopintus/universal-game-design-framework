...
type FieldProps={title:string;guide:string};
const Field=({title,guide}:FieldProps)=>(
  <div className="mb-6">
    <label className="block font-semibold mb-1">{title}</label>
    <p className="text-sm text-gray-500 mb-2">{guide}</p>
    <textarea className="w-full min-h-28 border rounded-lg p-3" placeholder="Write here..."/>
  </div>
);
const Section=({title,children}:{title:string;children:React.ReactNode})=>(
<section className="border rounded-xl p-6 mb-8 shadow-sm">
<h2 className="text-2xl font-bold mb-4">{title}</h2>{children}
</section>);
export default function UniversalGameDesignFramework(){
return <main className="max-w-5xl mx-auto p-8 space-y-8">
<h1 className="text-4xl font-bold mb-2">Universal Game Design Framework</h1>
<p className="text-gray-600 mb-8">High-level framework for designing games before writing a GDD.</p>
...
<Field title="Technology" guide="What engine, tools, or physical components will be used?"/>
<Field title="Production Scope" guide="How ambitious should this project be? What should intentionally remain out of scope?"/>
</Section>
...
<Field title="Desired Experience" guide="How should players describe the game after playing?"/>
...
<h3 className="text-xl font-semibold mb-3">Dynamics</h3>
...
<Field title="Environmental Storytelling" guide="How does the world communicate its history and mood?"/>
</Section>
...
<Field title="Short-Term Goals" guide="What should players accomplish in the next few minutes?"/>
<Field title="Mid-Term Goals" guide="What should players work toward over several sessions?"/>
...
<Field title="Victory Conditions" guide="How does the player win? Are there multiple paths?"/>
<Field title="Failure Conditions" guide="How can the player fail? What are the consequences?"/>
</Section>
...
<Field title="Trade-offs" guide="How should conflicts between good design options be resolved?"/>
</Section>

<Section title="11. Success Criteria">
<Field title="Player Experience" guide="What should playtesters consistently feel or say?"/>
...
<Field title="Things This Game Should Never Become" guide="What directions would betray the game's identity?"/>
</Section>
</main>}
